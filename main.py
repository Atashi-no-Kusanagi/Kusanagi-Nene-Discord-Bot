...
if not TOKEN:
    print("ERROR: No Discord token found. Set environment variable.")
...
async def chat(ctx, *, prompt: str = None):
    if not prompt:
        return await ctx.reply("You didn't...say anything.")

    async with ctx.typing():
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{AI_SERVER_URL}/chat", json={"prompt": prompt}) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        embed = discord.Embed(
                            title="KNene",
                            description=f"""KNene replies: {data["response"]}

(Generated by AI)""",
                        color=discord.Color.green())
                        await ctx.reply(embed=embed)
                    else:
                        await ctx.reply("KNene is currently sleeping or overloaded.")
        except Exception as e:
            await ctx.reply(f"Failed to reach KNene server: {e} | Notify @transmascneneisreal.")

@bot.command()
async def classify(ctx, *, prompt: str = None):
    if not prompt:
        return await ctx.reply("You didn't say anything!!")
    ...
    async with ctx.typing():
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{AI_SERVER_URL}/classify", json={"prompt": prompt}) as resp:
                    data = await resp.json()
                    percentage = data["score"] * 100
                    embed = discord.Embed(
                        title="NeneKlassifier classification",
                        description=f"""**NeneKlassifier returns a negativity confidence score of `{percentage:.3f}%`!**

(Generated by AI)""",
                    color=discord.Color.green())
                    await ctx.reply(embed=embed)
        except Exception:
            await ctx.reply("There was an error with NeneKlassifier. Notify @transmascneneisreal.")

@bot.command()
async def about(ctx, *, prompt: str = None):
    if not prompt:
